...
export default function LoginPage() {
  ...
  async function completeFirebaseServerSession(idToken: string) {
    ...
    if (!response.ok) {
      ...
      throw new Error(payload.error || t("errorSession"));
    }
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      try {
        ...
        throw new Error(t("errorFirebase"));
      }
      ...
      let msg = err instanceof Error ? err.message : t("errorGeneric");

      if (code === "auth/invalid-credential") {
        msg = t("errorInvalidCredential");
      } else if (code === "auth/email-already-in-use") {
        msg = t("errorEmailInUse");
